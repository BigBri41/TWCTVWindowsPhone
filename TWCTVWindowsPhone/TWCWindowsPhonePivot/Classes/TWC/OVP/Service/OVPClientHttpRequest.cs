...
public class OVPClientHttpRequest
{
    ...
    private static WebRequestResult HttpRequest(string url, int timeoutMilliSeconds, bool isClientRequest)
    {
        ...
        Task<WebResponse> task = Task.Factory.FromAsync<WebResponse>(new Func<AsyncCallback, object, IAsyncResult>(request.BeginGetResponse), new Func<IAsyncResult, WebResponse>(request.EndGetResponse), request);
        try
        {
    ...
    public static JsonRequestResult<T> JsonRequest<T>(string url, int timeoutMilliSeconds, bool isClientRequest, Func<string, string> processJson = null) where T: class
    {
        Action action = null;
        string json;
        HttpWebResponse response;
